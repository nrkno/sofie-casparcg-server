...
void frame_producer_registry::register_producer_factory(std::wstring               name,
                                                        const producer_factory_t&  factory,
                                                        const help_item_describer& describer)
{
...
{
    CASPAR_THROW_EXCEPTION(user_error() << msg_info(L"No variable called " + name + L" found in " + print()));
}
...
draw_frame frame_producer_registry::create_thumbnail(const frame_producer_dependencies& dependencies,
