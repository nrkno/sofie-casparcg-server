...
class decklink_timecode : public IDeckLinkTimecode
{
  ...
  public:
    ...
    HRESULT GetString(String* timecode) override { return S_FALSE; }
...
class decklink_frame : public IDeckLinkVideoFrame
{
  ...
  public:
    decklink_frame(core::const_frame              frame,
            ...
            if (static_cast<int>(frame_.image_data().size()) != format_desc_.size) {
                ...
                *buffer = data_.data();
                ...
                if (data_.empty()) {
                    ...
                    aligned_memshfl(data_.data(),
                                    frame_.image_data().begin(),
                                    frame_.image_data().size(),
                                    0x0F0F0F0F,
                                    0x0B0B0B0B,
                                    0x07070707,
                                    0x03030303);
    ...
    const core::audio_buffer& audio_data() { return frame_.audio_data(); }

    int64_t get_age_millis() const { return frame_.get_age_millis(); }
};
    ...
    , boost::noncopyable
{
  ...
  public:
        ...
        , in_channel_layout_(in_channel_layout)
    {
        ...
        enable_video(get_display_mode(
            output_, format_desc_.format, bmdFormat8BitBGRA, bmdVideoOutputFlagDefault, will_attempt_dma_));
    ...
    void schedule_next_video(core::const_frame frame, core::frame_timecode timecode)
    {
        if (key_context_) {
            auto key_frame = wrap_raw<com_ptr, IDeckLinkVideoFrame>(
                new decklink_frame(frame, format_desc_, timecode, true, will_attempt_dma_));
        ...
        auto fill_frame = wrap_raw<com_ptr, IDeckLinkVideoFrame>(
            new decklink_frame(frame, format_desc_, timecode, config_.key_only, will_attempt_dma_));
...
void describe_consumer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.definitions()
        ->item(L"device_index",
               L"The Blackmagic video card to use (See Blackmagic control panel for card order). Default is 1.")
        ->item(L"keyer",
               L"If given tries to enable either internal or external keying. Not all Blackmagic cards supports this. "
               L"There is also a third experimental option (EXTERNAL_SEPARATE_DEVICE_KEY) which allocates device_index "
               L"+ 1 for synhronized key output.")
    ...
    sink.example(L">> ADD 1 DECKLINK 1 EMBEDDED_AUDIO\n"
                 L">> ADD 1 DECKLINK 2 KEY_ONLY",
                 L"uses device with index 1 as fill output with audio and device with index 2 as key output.");
    sink.example(L">> ADD 1 DECKLINK 1 EXTERNAL_SEPARATE_DEVICE_KEY EMBEDDED_AUDIO",
